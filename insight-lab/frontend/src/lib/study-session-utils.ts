...
const DOCUMENT_QUIZ_STEP_TYPES = new Set(["quiz"]);
...
/** Step id for document-level quiz submit during a tracked session. */
export function resolveDocumentQuizStepId(session: StudySessionRecord | null | undefined): string | null {
  if (!session || session.status !== "active") {
    return null;
  }

  const inProgress = session.steps.find(
    (step) => DOCUMENT_QUIZ_STEP_TYPES.has(step.step_type) && step.status === "in_progress",
  );
  if (inProgress) {
    return inProgress.id;
  }
  ...
  const current = session.steps.find(
    (step) =>
      step.step_index === session.current_step_index && DOCUMENT_QUIZ_STEP_TYPES.has(step.step_type),
  );
  if (current && current.status !== "completed" && current.status !== "skipped") {
    return current.id;
  }
